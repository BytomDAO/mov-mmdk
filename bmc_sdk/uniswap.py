...
class Uniswap(EthClient):
    ...
    def __init__(
        ...
        """
        super(Uniswap, self).__init__(address=address, private_key=private_key, network=network, provider=provider,
                                      web3=web3)
    ...
    def check_and_go_aprove(self, token: AddressLike):
        if ETH_ADDRESS != token:
            is_approved = self._is_approved(token)
            log_service_manager.warning(f"Approved? {token}: {is_approved}")
            if not is_approved:
                log_service_manager.write_log(f"[check_and_go_aprove] go approve token:{token}")
                self.approve(token)

    @supports([2])
    def remove_liquidity_v2(self,
                            pair: AddressLike,
                            tokenA: AddressLike,
                            tokenB: AddressLike,
                            liquidity: Wei,
                            to: AddressLike) -> HexBytes:
        self.check_and_go_aprove(pair)
        self.check_and_go_aprove(tokenA)
        self.check_and_go_aprove(tokenB)
        if str_to_addr(tokenA) == ETH_ADDRESS or str_to_addr(tokenB) == ETH_ADDRESS:
            if str_to_addr(tokenA) == ETH_ADDRESS:
                func_params = [str_to_addr(tokenB), liquidity, 1, 1, str_to_addr(to),
                               self._deadline()]
            else:
                func_params = [str_to_addr(tokenA), liquidity, 1, 1, str_to_addr(to),
                               self._deadline()]
            function = self.router.functions.removeLiquidityETH(
                *func_params
            )
            return self.build_and_send_tx(function)
        else:
            func_params = [str_to_addr(tokenA), str_to_addr(tokenB), liquidity, 1, 1, str_to_addr(to), self._deadline()]
            function = self.router.functions.removeLiquidity(
                *func_params
            )
            return self.build_and_send_tx(function)

    # ------ Approval Utils ------------------------------------------------------------
    def _approve(self, token: AddressLike, contract_addr: AddressLike, max_approval: Optional[int] = None) -> None:
    ...
    def approve(self, token: AddressLike, max_approval: Optional[int] = None) -> None:
        ...
        return self._approve(token, contract_addr, max_approval)

    def _is_token_approve_contract(self, token: AddressLike, contract_addr: AddressLike) -> bool:
    ...
    def _is_approved(self, token: AddressLike) -> bool:
        ...
        return self._is_token_approve_contract(token, contract_addr)
