...
def run(token_symbol, input_txt):
    ...
    with open(keystore_file_path) as keyfile:
        ...
        for to_address, amount in info_arr:
            send_amount_dic[ind] = (to_address, amount)
            to_work_dic[ind] = ""
            ind = ind + 1
        ...
        while True:
            ...
            for ind, tx_id in now_loop_arr:
                to_address, amount = send_amount_dic[ind]
                try:
                    if tx_id:
                        ...
                        if transaction_info:
                            ...
                            if status:
                                log_service_manager.write_log(f"[run] ind:{ind+1} to_address:{to_address} amount:{amount} success!")
                                if ind in to_work_dic.keys():
                                    del to_work_dic[ind]
                            else:
                                ...
                                if transaction_hash:
                                    to_work_dic[ind] = HexBytes(transaction_hash).hex()
                    else:
                        transaction_hash = client.transfer(to_address, amount, token_symbol)
                        if transaction_hash:
                            to_work_dic[ind] = HexBytes(transaction_hash).hex()
